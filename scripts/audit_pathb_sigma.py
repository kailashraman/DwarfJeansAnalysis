...
def main():
    ...
    for r in reg:
        ...
        ads_url = (f"https://ui.adsabs.harvard.edu/abs/{ref_vlos}/abstract"
                   if ref_vlos else "")
        rows.append({
            "lvdb_key": key,
            "ref_vlos": ref_vlos,
    ...
    md = ["# Path B σ_los audit: LVDB-quoted vs constant-σ posterior",
          ...
          "",
          ("**Caveat (Δσ jeff−loguni column).** The constant-σ prior kwarg "
           "was added in commit `9991086` (2026-05-11); all existing "
           "production runs in `results/production/` predate that commit "
           "and used the default Jeffreys prior for the constant-σ "
           "computation, regardless of the 7D-Jeans prior. The loguniform "
           "column in this table is therefore identical to the Jeffreys "
           "column. Re-running production with the prior-aware code will "
           "populate it with a genuine loguniform constant-σ posterior."),
          "",
          ("Sorted by |tension| under the Jeffreys prior. Generated by "
           "`scripts/audit_pathb_sigma.py`. The `ref_vlos` column links to "
           "the paper's ADS abstract (which carries the arXiv link)."),
          "",
