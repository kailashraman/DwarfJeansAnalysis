...
| 2026-05-05 | Segue 1 test gains `USE_P_WEIGHTS` and `FIX_R_P_ARCMIN` toggles plus a multi-epoch Simon combiner | Local toggles in `Segue1_test/run_segue1.py` only; library footprint limited to a `fix_r_p_arcmin: bool` kwarg on `make_loglike_with_nuisances`/`run_inference`. The Simon ingest now inverse-variance-combines per-star epochs (V matches Pace exactly; eV not binary-inflated, so on ~half of multi-epoch stars eV is below Pace's binary-aware value — a documented limitation). `USE_P_WEIGHTS=False` collapses surviving p_i to 1; `FIX_R_P_ARCMIN=(mean,sigma)` puts a direct Normal prior on the angular Plummer scale. Outputs gain `_nop` / `_fixrp` suffixes. See `segue1_test.md` § Toggles. |
