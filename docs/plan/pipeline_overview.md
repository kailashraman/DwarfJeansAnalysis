...
**Tidal-radius computation.** Per draw, `r_t` solves the Tormen (1998) / Springel et al. (2008) radial tidal-radius condition (factor `2 − dlnM_host/dlnr`, swappable to the Jacobi `3 −`) in the **SatGen m12** MW host (`Mvir = 1e12 M⊙`, `c ≈ 11.5`; NFW), using the galactocentric distance from `(ra, dec, d)`. It depends only on `(r_s, ρ_s, D, host)`, so it is computed for every dwarf — the former fixed `r_t = 1 kpc` and the unresolved-σ_los fallback are dropped. Implementation and validation live in `stage3.md` (`src/dwarfjeans/jd/tidal.py`).
