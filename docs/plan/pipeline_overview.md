...
| 2026-05-01 | J/D push knobs validated. `summarize_jd` defaults `(thin_to=500, n_R=64, n_u=128)`; `analyze_asimov.py` uses `(n_R=96, n_u=256)`; production driver `run_production.py` uses the `J_D_factors` defaults `(n_R=128, n_u=512)`. | Validated against `scipy.integrate.quad` to better than 10⁻³ relative error over the MW-dwarf parameter range. The `run_jd_summary.py` referenced earlier was not migrated; J/D is computed inline by `run_ufd_population.py` (MC sweep) and `run_production.py` (production). |
