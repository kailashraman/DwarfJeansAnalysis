...
- Required LVDB fields per galaxy: `rhalf`, `distance_modulus`, `apparent_magnitude_v` (and their errors). These are present for every confirmed MW dwarf in v1.0.5; if any are missing for a future system, drop that galaxy. `vlos_systemic` and `vlos_sigma` are **not** required for ingest:
  - `vlos_systemic` is **not** used to center the V prior — Stages 1 and 2 center on the IVW mean of the post-selection velocities (default ±10 km/s). The registry value is logged for diagnostics only. The optional per-galaxy `vlos_prior_halfwidth_kms` registry column (default 10) can be widened for small samples or unusual velocity distributions; `run_production.py` reads it via `row.get(..., V_HALFWIDTH)` and plumbs it to `priors.py`. Add the column to `galaxies.ecsv` by hand if you need an override.
...
**Reporting angles.** `θ_max ∈ {0.1°, 0.2°, 0.5°, α_c}` for J (where `α_c = 2 r_½,3D / d` is the Wolf+2010 critical angle); `θ_max ∈ {0.1°, 0.2°, 0.5°, α_c/2}` for D. Plus the **95% J-factor containment angle** `θ₉₅(J)` (half-angle of the cone holding 95% of `J(<r_t)`), computed with exact LOS geometry (`j_containment`, ported from SatGen).

**Tidal-radius computation.** Per draw, `r_t` solves the Tormen (1998) / Springel et al. (2008) radial tidal-radius condition (factor `2 − dlnM_host/dlnr`, swappable to the Jacobi `3 −`) in the **real Milky Way** host — Gala's `MilkyWayPotential2022` (NFW halo + Hernquist bulge + nucleus + MN disk; spherically-enclosed `M(<D)`), implemented in `src/dwarfjeans/jd/mw_host_model.py` — using the galactocentric distance from `(ra, dec, d)`. (The legacy fixed-mass SatGen m12 NFW host, `Mvir = 1e12 M⊙`, `c ≈ 11.5`, is retained as `HostNFW` to reproduce pre-change chains.) It depends only on `(r_s, ρ_s, D, host)`, so it is computed for every dwarf — the former fixed `r_t = 1 kpc` and the unresolved-σ_los fallback are dropped. Implementation and validation live in `stage3.md` (`src/dwarfjeans/jd/tidal.py`).
...
|---|---|---|---|---|
| 1 | **Membership-probability weighting in the likelihood** | P&S eq. 8 carries `p_i` inside a product of per-star Gaussians as a multiplicative prefactor `p_i / √[2π(...)]`. As written, this rescales the likelihood by a global constant `∏ p_i` and does not affect the inference — the gradients, posterior shape, and MAP are identical to `p_i ≡ 1`. For `p_i` to weight each star's contribution to the inference, it has to multiply terms in the *sum* of log-likelihood contributions, so we depart from eq. 8 as printed. The distinction does not arise in P&S's own application, which hard-cuts at ingest and sets all surviving stars to `p_i = 1`. | Members enter the log-likelihood as `∑_i p_i · ln L_i(V, σ_los)`, with `p_i` as a continuous weight on each star's log-contribution. The membership cut is still hard at ingest (per P&S §2 thresholds); the weighting acts on surviving stars, who may carry continuous `p_i` from the source paper. | The summed form is mathematically what "weight by membership probability" means. In the P&S-replication regime where surviving `p_i ≡ 1`, the two forms agree exactly, so reproduction of Table A1 is unaffected. Our weighting only differs from P&S when membership probabilities are propagated as continuous values (notably Segue 1 with Simon+11 Bayesian probabilities). |
...
| 8 | **Pipeline scope** | P&S 2018 covers MW, M31, and Local Field dwarfs. | We restrict to MW satellites only (including LMC/SMC subsystems, walked via the LVDB host chain). | The SatGen m12 MW host-mass profile we use for the tidal radius is MW-specific; M31 satellites would need an M31 host profile, and Local Field dwarfs would need a different `r_t` convention. None of these are blockers; they're deferred to a future iteration to keep initial scope contained. |
...
| 2026-04-30 | Optional per-galaxy `vlos_prior_halfwidth_kms` registry column, default 10 km/s; wired through `run_production.py` → `priors.py` 2026-05-08 (add column to `galaxies.ecsv` by hand to override) | Allows expansion for individual galaxies per P&S 2018 footnote 6 |
...
| 2026-05-01 | J/D push knobs validated. `analyze_asimov.py` uses `(n_R=96, n_u=256)`; production driver `run_production.py` uses the `J_D_factors` defaults `(n_R=128, n_u=512)`. | Validated against `scipy.integrate.quad` to better than 10⁻³ relative error over the MW-dwarf parameter range. The earlier `run_jd_summary.py` / `summarize_jd` helper was not migrated and has been removed; J/D is computed inline by `run_production.py` (production). |
...
| 2026-05-04 | Conditional Jeffreys prior on `(ln ρ_s, ln r_s)` at fixed β as the default for Stage 2 | Replaces the previous log-flat-on-`(log r_s, log ρ_s)` truncated to a box. Fisher determinant of the Walker+2006 likelihood; `½ ln D` added to the log-likelihood per call. Derived in `jeffreys_jeans_derivation.md`. On Segue 1 the prior shifts ρ_s up by ~1.4 dex (low-ρ_s tail correctly suppressed) and Jeans-derived σ_los at R_½ now matches the prior-independent Walker constant-σ result (3.86 vs 3.96 km/s; log-flat fit was 2.84). Selected by `prior_name` (default `'jeffreys'`); `'loguniform'` or `'uniform'` revert to the previous log-flat behaviour. `run_ufd_population.py` now passes `prior_name='jeffreys'`; the 15-realization MC calibration documented in `stage2.md` was produced under log-flat priors and is pending a re-run. See `stage2.md` § Priors and `segue1_test.md` § Latest results. |
| 2026-05-05 | Segue 1 test gains `USE_P_WEIGHTS` and `FIX_R_P_ARCMIN` toggles plus a multi-epoch Simon combiner | Local toggles in `tests/integration/run_segue1.py` only; library footprint limited to a `fix_r_p_arcmin: bool` kwarg on `make_loglike_with_nuisances`/`run_inference`. The Simon ingest now inverse-variance-combines per-star epochs (V matches Pace exactly; eV not binary-inflated, so on ~half of multi-epoch stars eV is below Pace's binary-aware value — a documented limitation). `USE_P_WEIGHTS=False` collapses surviving p_i to 1; `FIX_R_P_ARCMIN=(mean,sigma)` puts a direct Normal prior on the angular Plummer scale. Outputs gain `_nop` / `_fixrp` suffixes. See `segue1_test.md` § Toggles. |
| 2026-05-05 | `jeffreys_jeans_derivation.md` carries `p_i` through the Fisher sums | Doc-only fix: the derivation had stated the likelihood as `∏_i p_i · 𝒩_i` (multiplicative prefactor → no `p_i` in Fisher), but the code uses the Walker pseudo-likelihood `Σ_i p_i ln 𝒩_i` (membership as exponent → linear `p_i` in Fisher). Doc updated to declare the pseudo-likelihood form and propagate `p_i` through eq 89, 97, 101, 153 and the practical recipe. No code change. |
