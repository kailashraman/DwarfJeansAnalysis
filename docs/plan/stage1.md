...
- **`V`:** uniform prior centered on the inverse-variance-weighted mean of the post-selection velocities, `V̄_IVW = Σ V_i/σ_{ε,i}² / Σ 1/σ_{ε,i}²`, with half-width 10 km/s (`V̄_IVW − 10 ≤ V ≤ V̄_IVW + 10`). Centering on the data rather than on the LVDB `vlos_systemic` value avoids stale/sample-mismatch failures (Pegasus III hit the lower wall of a registry-centered prior in QA). The half-width is a per-galaxy override column in the registry (`vlos_prior_halfwidth_kms`, default 10) read by `scripts/run_production.py` and plumbed through to the prior builders in `src/dwarfjeans/jeans/priors.py`; widen for small samples or unusual velocity distributions (P&S 2018 footnote 6). Same convention as Stage 2.
