...
- **`V`:** uniform prior centered on the literature systemic velocity `V_lit` (from the LVDB `vlos_systemic` column when present, else from the source spectroscopic paper), `V_lit − 10 ≤ V ≤ V_lit + 10` km/s. The half-width (10 km/s) is set as a per-galaxy override field in the registry (`vlos_prior_halfwidth`), so the standard width is the default and individual galaxies with small samples or unusual velocity distributions can have it expanded as needed (P&S 2018 footnote 6 notes such expansion is sometimes required). Same convention as Stage 2.
