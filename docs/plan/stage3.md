...
Plus the **exact-geometry total factors** `J(θ=π/2)=J(<r_t)` and `D(θ=π/2)=D(<r_t)` (`log10_J_pi2`, `log10_D_pi2`), the **95% containment angles** `θ₉₅(J)` and `θ₉₅(D)` — the half-angle of the cone containing 95% of the factor integrated out to `r_t` — and the per-draw chains for the tidal radius `r_t` and the galactocentric distance `D_GC`. These use **exact** line-of-sight geometry (not the small-angle approximation, which is invalid at the full angular extent; see "Angular containment" below), ported from the SatGen `compute_J_and_containment`.
...
- **Host: real Milky Way (default).** The default host is Gala's `MilkyWayPotential2022` (NFW halo + Hernquist bulge + nucleus + 3-component MN disk), implemented self-contained (numpy+scipy, no gala at runtime) in `src/dwarfjeans/jd/mw_host_model.py` (`MWPotential2022Host`, reproduces gala 1.11 to <0.2%). `M_host(<D)` is the true spherically-enclosed mass `∫ρ dV` (the self-consistent convention for the spherical tidal criterion; agrees with the force-based form to <1% at D ≳ 20 kpc where the halo dominates); `dlnM/dlnr` is analytic for the spherical parts and shell-integrated for the disk. Relative to the old m12 NFW, r_t is ~20–45% smaller at D ≲ 50 kpc (disk+bulge steepen the inner tidal field), shrinking to ~10% by D ~ 100 kpc. The legacy fixed-mass NFW host (SatGen `m12res8-Diemer`: `Mvir = 1e12 M⊙`, `Rvir = 258.9 kpc`, `c ≈ 11.5`; `dlnM/dlnr = h(x)/g(x)` via `nfw_g`/`nfw_h`) is retained as `HostNFW` to reproduce pre-change chains. Any object exposing `M_enc(D)`/`dlnM_dlnr(D)` is a valid host (`factor`/host are swappable). See `docs/plan/mw_host_model.md`.
...
**Implemented** in `factors.py` (`j_containment`, `j_aperture_and_containment`, and the D analogues `d_containment`, `d_aperture_and_containment`), ported from the validated SatGen `compute_J_and_containment`. Reports `θ₉₅(J)`: the half-angle of the cone containing 95% of `J(<r_t)`. Uses the **exact** line-of-sight geometry (`los_radius`, law-of-cosines half-angle form, valid to θ = π/2) rather than the small-angle approximation, because `θ₉₅` reaches several tenths of a degree to ~1° for nearby dwarfs. The NFW is truncated at `r_t`, so the angular profile self-limits at `θ_max = arcsin(r_t/d)`; `dJ/dθ` is built by deterministic Gauss–Legendre LOS quadrature and `θ₉₅` read off the normalised cumulative. Cross-checks: `J(<0.5°)` agrees with the small-angle `J_D_factors` to <0.01 dex, and `J(<r_t)` matches an independent `scipy.integrate.dblquad` to <0.01 dex.

**Total (θ=π/2) factors.** The full exact-geometry `J(<r_t)` — i.e. `J(θ=π/2)`, since ρ=0 beyond `r_t` — and its D analogue are saved as posteriors (`log10_J_pi2`, `log10_D_pi2` in `derived.npz`/`summary.csv`), plus the D containment angle `θ₉₅(D)`. These are the large-angle-correct totals: the small-angle approximation (`R = d·θ`) behind the fixed-angle J/D is invalid at the full angular extent, which reaches several degrees for nearby, large-`r_t` systems. `d_containment` mirrors `j_containment` with the `∫ρ dℓ` column (power 1) instead of `∫ρ² dℓ` (power 2); since D weights ρ (not ρ²) it is less centrally concentrated, so `θ₉₅(D) > θ₉₅(J)`. The θ→0 cusp differs: `dD/dθ ~ θ ln(1/θ) → 0` (vs `dJ/dθ → const` for the γ=1 NFW cusp), so the `0·∞` on-axis lift is applied only for J. (The lift shifts `J(<r_t)` by ~0.2%; for D the no-lift choice is formal — <1e-5 dex at grid resolution.) **Units subtlety:** `j_/d_containment` return values already in GeV²/cm⁵ · GeV/cm², so `derive` takes `log10` directly — unlike the fixed-angle path, which adds `LOG10_J/D_FAC` to a raw Msun-unit value.
...
The J/D push uses `thin_to=200, n_R=48, n_u=96`, calibrated against `(thin_to=500, n_R=64, n_u=128)` — population diagnostics agree to within 15-realization MC noise — for an ~8× speedup (~30 s wall vs. ~4 min for the full sweep). Result artifacts: `results/tests/ufd_population/ufd_pop_jd.json`, `results/tests/ufd_population/ufd_pop_jd_diagnostics.json`.
...
The Asimov chain (`results/tests/ufd_population/compact_ufd_asimov.npz`, 1803 equal-weight samples; constructed in `stage2.md` § Asimov dev-loop check) is informative enough by itself to localize the J-factor bias. The diagnostics below resolve two questions: (i) is `M(r_½)` recovered cleanly, and (ii) where does the +0.15–0.23 dex Asimov J-bias actually come from. Reproduce with `python tests/integration/analyze_asimov.py` (full diagnostic), output saved to `results/tests/ufd_population/asimov_chain_diagnostics.npz` and `results/tests/ufd_population/asimov_diagnostic.png`.
