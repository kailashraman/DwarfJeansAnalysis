...
**Prior used for this calibration:** log-flat on `(log_10 r_s, log_10 ρ_s)` (the previous default). The conditional-Jeffreys prior introduced for the Segue 1 analysis has not yet been validated through this MC; `run_ufd_population.py` now passes `prior_name="jeffreys"` (current default) so the next run will produce the under-Jeffreys numbers. Qualitatively, in the resolved-dispersion regime that this MC sits in (`A_i ≫ ε_i²`), the Jeffreys term becomes ρ_s-independent, so the recovery should be similar; this needs to be verified rather than assumed.
