...
**SatGen-conditioned prior.** `'satgen'` draws `(log_10 r_s, log_10 ρ_s)` from the ΛCDM-with-tidal-stripping subhalo catalog `m12res8_10k_Diemer+scatter_sim.h5` in the companion `SatGen_Dwarf` repo. Each halo's `(v_max, r_max)` is mapped to NFW `(r_s, ρ_s)` via `r_s = r_max / x_max` and `ρ_s = (v_max/r_max)² · x_max³ / (4π G μ(x_max))` with `μ(x)=ln(1+x)−x/(1+x)` and `x_max ≈ 2.16258` (the root of the NFW `v_c` derivative). The numerical constants are `r_s/r_max = 0.46241029979236` and `ρ_s = 1.7212585601570 · (v_max/r_max)² / G`. The prior is then `log_10 r_s` from the SatGen marginal CDF (inverted via `np.interp`) and `log_10 ρ_s | log_10 r_s ~ N(μ(log r_s), σ(log r_s))`, where `μ, σ` are tabulated as the per-bin sample mean and std (ddof=1) of `log_10 ρ_s` in 30 equal-width bins over `log_10 r_s ∈ [-2, 1]`. Pre-baked at `data/satgen_prior/m12res8_diemer_scatter.npz`; rebuilt by `scripts/build_satgen_prior_table.py`. Pooled-residual diagnostics give skew = +0.13 and excess kurtosis = +0.17 across 2.4×10⁶ halos — the lognormal approximation is faithful in the bulk; a 2D KDE form would be the follow-up if higher-moment structure ever matters (tracked in [`docs/open-issues.md`](../open-issues.md) #5). `V` and `β̃` priors are unchanged; `log10_rs_min` truncates the marginal CDF and re-renormalises to `[0, 1]`.
...
This is justified by a scale separation between the fitted tracers (selected within `2 r_half`) and `r_t`: σ_los at the data radii is set by mass interior to the stars, and the Plummer `r⁻⁵` tail suppresses the tracer weight beyond `r_t`. Verified across all 39 MW dwarfs (MW-host `r_t`) via the 3D tracer-mass fraction beyond `r_t`, `f = 1 - r_t³/(r_t²+r_p²)^{3/2}`, evaluated **per posterior draw** (`r_t_kpc` paired with `r_p_chain[idx_jd]` from `derived.npz`; `scripts/scale_sep95.py`). The numbers below are the **`jeffreys` posterior**; because `r_t` tracks the inferred mass, `f` is prior-dependent (`--prior` selects other runs), and the headline percentages swing by ~3× across priors — e.g. Crater 2's median ranges 6.6% (`satgen_shmr_fattahi18`) to 18.5% (`loguniform`). The *ranking* of exceptions is prior-robust; the single quoted values are not.

`f` is steeply nonlinear in `r_t`, so the median posterior understates the upper tail — both summaries are quoted below. The qualitative conclusion is the same under either, but the "<0.5%" headline is median-specific and degrades at 95%:

| summary   | <0.5%     | <1%       | <2%       | >5%     |
|-----------|-----------|-----------|-----------|---------|
| median    | **32/39** | 35/39     | 36/39     | 2/39    |
| 95th pct  | **24/39** | 30/39     | 34/39     | 3/39    |

Most are ≪0.1% (Segue 1 0.07%/0.21%, Draco 0.05%/0.17%, quoted median/95th). Three systems are clear exceptions, where `r_t` and the tracer extent become comparable: **Crater 2 (18% / 30%), Antlia 2 (6.5% / 10.1%), Bootes 3 (3.1% / 10.0%)**. Below them a tail sits near 1–2% at 95% (Tucana 2 1.1%/3.6%, Bootes 1 0.9%/2.1%, Hydrus 1 0.6%/1.5%, Carina 2 0.4%/1.4%, Sextans 1 0.7%/1.3%, Bootes 2 0.3%/1.1%). For the three exceptions a truncated-σ_los *would* shift the fit.

**Bootes 3 is a clear exception, not a marginal case** — its median (3.1%) reads as tail-adjacent but its posterior is the most skewed among the three exceptions (3.2× median-to-95th; more-skewed systems exist but sit at negligible `f` where the skew is immaterial), and at 95% it ties Antlia 2 at ~10%. Do not classify it from the median alone. Those are, however, the same systems where the equilibrium single-component Jeans assumption is itself least reliable (Antlia 2 shows a clear tidal-disruption gradient; Bootes 3 is a disrupting stream progenitor). Adopting the real-MW host slightly sharpens this, since it lowers `r_t` ~20–45% at D≲50 kpc relative to the old m12 NFW.
...
The individual halo parameters `log r_s`, `log ρ_s`, and `log(ρ_s · r_s³)` are now meaningfully biased (|med bias| = 0.6–1.1 dex) with strongly non-normal z-distributions (KS p < 10⁻³). The Stage 3 / Stage 4 reporting strategy still follows from `M(r_½, 3D)` as the headline well-constrained quantity, but the Jeffreys-vs-loguniform regression here is a finding worth flagging for follow-up (tracked in [`docs/open-issues.md`](../open-issues.md) #3).
...
Some classical dwarfs (notably Carina, Fornax, Sculptor) show measurable line-of-sight velocity gradients across the field, attributable to a mix of intrinsic rotation, tidal streaming, and the perspective-motion signal of the bulk velocity. Antlia II is a particularly stark case: Ji et al.\ 2021 (the same paper we ingest for its kinematics) report a clear gradient consistent with ongoing tidal disruption, and after perspective subtraction the residual streaming is *not* negligible. The Jeans likelihood as currently written assumes a single zero-mean (after subtracting the systemic $V$) isotropic-in-position dispersion, with no spatial trend in $\langle v_\mathrm{los} \rangle$.

**Status: not yet implemented.** Tracked in [`docs/open-issues.md`](../open-issues.md) #2. Options under consideration:
