...
- **Equal array lengths used as a proxy for "already aligned."** A reindex guarded by a size comparison (`if a.size != b.size: a = a[idx]`) is only correct if *same length* provably implies *same order* for every reachable state of `idx`. That equivalence is typically an accident of a thinning helper elsewhere returning the identity `np.arange(N)` exactly when `N <= thin`, so the sizes-equal case also happens to be the already-aligned case. The moment that fallback becomes any non-identity permutation of the same length, the guard silently stops reindexing: no exception, no NaN, values individually plausible, correlations between the two arrays scrambled. Defense: index unconditionally through the saved index array, and gate on the *presence* of that array (`"idx" in npz.files`) rather than on lengths. Flag any `if x.size != y.size:` conditional wrapping an index operation — especially when a sibling function in the same file already does the explicit, presence-gated thing.
