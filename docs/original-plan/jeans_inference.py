...
def make_prior_transform(V_center: float):
    ...
    def prior_transform(u: np.ndarray) -> np.ndarray:
        ...
        x[1] = LOG10_RS_BOUNDS[0] + (LOG10_RS_BOUNDS[1] - LOG10_RS_BOUNDS[0]) * u[1]
...
def make_loglike(R: np.ndarray, V: np.ndarray, sigma_eps: np.ndarray,
    ...
    def loglike(theta: np.ndarray) -> float:
        ...
        # log-Gaussian, summed (membership-weighted; here all p_i = 1)
...
def run_inference(
    ...
    prior_transform = make_prior_transform(V_center)
...
def summarize_jd(samples_eq: np.ndarray, truth: dict,
    ...
    for k, arr in derived.items():
        ...
        sigma_hi = q84 - q50
        true_val = truth_map[k]
        # one-sided z: how many sigma is the truth from the median?
        if true_val < q50:
            z = (true_val - q50) / max(sigma_lo, 1e-12)
        else:
            z = (true_val - q50) / max(sigma_hi, 1e-12)
